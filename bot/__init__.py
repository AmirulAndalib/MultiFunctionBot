...
class ENV_VARS(object):
    ...
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "5465691492:AAGpEQqQN5LJI_wZhfCQEVK0yTApgi9WV94")
...
class CMD(object):
    ...
    LINKV = ["linkvertise", f"linkvertise@{handler}"]
    ADFL = ["adfly", f"adfly@{handler}"]
    SRGN = ["sirigan", f"sirigan@{handler}"]
    OUOT = ["ouo", f"ouo@{handler}"]
    SHST = ["shorte", f"shorte@{handler}"]
    AIO = ["multi", f"multi@{handler}"]
    RKLK = ["rocklink", f"rocklink@{handler}"]
